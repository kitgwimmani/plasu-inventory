import React, { useEffect, useState } from "react";
import { Row, Card, Table, Alert, Spinner, Button } from "react-bootstrap";
import { Link } from "react-router-dom";
import api from "../../api/axios";
import StatCard from "../../components/StatCard";
import { useAuth, ROLE_LABELS } from "../../context/AuthContext";
import { formatDate } from "../../utils/formatDate";

export default function ClearanceDashboard() {
  const { user } = useAuth();
  const [stats, setStats] = useState(null);
  const [error, setError] = useState("");

  useEffect(() => {
    api
      .get("/dashboard")
      .then((res) => setStats({ awaitingMyClearanceSignoff: [], ...res.data.stats }))
      .catch((err) => setError(err?.response?.data?.error || "Could not load dashboard."));
  }, []);

  if (error) return <Alert variant="danger">{error}</Alert>;
  if (!stats) {
    return (
      <div className="text-center py-5">
        <Spinner animation="border" style={{ color: "#0f6b2c" }} />
      </div>
    );
  }

  return (
    <>
      <h4 className="mb-1">{ROLE_LABELS[user.role]} Dashboard</h4>
      <p className="text-muted">
        Review the stock receipts the Head of Store submits and record your clearance signature.
        You can also view and print inventory and reports.
      </p>

      <Row>
        <StatCard
          label="Awaiting My Signature"
          value={stats.awaitingMyClearanceSignoffCount}
          warn={stats.awaitingMyClearanceSignoffCount > 0}
          md={4}
          to="/clearance?status=pending"
        />
        <StatCard label="Cleared by Me" value={stats.clearedByMeCount} md={4} />
      </Row>

      <Card className="plasu-card p-3 mt-2">
        <h6>Clearance Requests Awaiting My Signature</h6>
        <Table responsive hover size="sm" className="table-plasu mb-0">
          <thead>
            <tr><th>Ref No.</th><th>Range</th><th>Receipts</th><th>Submitted By</th><th></th></tr>
          </thead>
          <tbody>
            {stats.awaitingMyClearanceSignoff.map((c) => (
              <tr key={c.id}>
                <td>{c.ref_no}</td>
                <td>{formatDate(c.date_from)} – {formatDate(c.date_to)}</td>
                <td>{c.receipt_count}</td>
                <td>{c.created_by_name}</td>
                <td>
                  <Button as={Link} to={`/clearance/${c.id}`} size="sm" className="btn-plasu">
                    Review &amp; Sign
                  </Button>
                </td>
              </tr>
            ))}
            {stats.awaitingMyClearanceSignoff.length === 0 && (
              <tr><td colSpan={5} className="text-center text-muted">Nothing awaiting your signature.</td></tr>
            )}
          </tbody>
        </Table>
      </Card>
    </>
  );
}
